...
下面这张动图是 `jobfindsme` 在本机对一个真实猎聘请求的完整运行过程：
从一句自然语言 → 简历本地解析 → 实时调用猎聘 API → 输出带匹配度与投递链接的岗位列表。
每一条岗位、每一个数字都由 `jobfindsme` 自身代码生成，不是示意图。
...
<details>
<summary>📸 等不及动画？先看一张静态效果截图（实时渲染，非 mock）</summary>
<br>
<p align="center">
...
> 💡 图中 `BOSS直聘` 与 `猎聘` 两个平台一起出结果 —— 录制时本机已通过 `jobfindsme setup` 登录 BOSS 专用 Chrome。
...
几秒后，Agent 返回匹配的岗位列表（下面是真实运行的输出节选）：

```text
✅ 猎聘·上海 ✓ (1.0s)
🆕 新增 20 个匹配岗位（原始 42 → 过滤后 42）

1. [新增] AI应用工程师（Agent开发）｜某知名公司｜上海｜社招｜正式｜40-60k·15薪
   匹配度：63%（信号匹配，非录用概率）
   技能：Agent ｜ 学历：不限
   投递链接：https://www.liepin.com/a/78197081.shtml

2. [新增] AI应用工程师｜上海韦晴软件科技有限公司｜上海-静安区｜社招｜正式｜20-25k
   匹配度：20%（信号匹配，非录用概率）
   经验：1-3年 ｜ 学历：本科
