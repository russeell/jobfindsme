...
TOOL_DEFINITIONS = (
    ...
    ToolDefinition(
        ...
        (
            ...
            "local DB only.  "
            "Returns hard-filtered, coarse-ranked structured facts in "
            "structuredContent.jobs: each job with title, company, location, "
            "salary, score, evidence (matched/missing skills, warnings), "
            "change state, and its apply URL; plus a compact factual "
            "summary. The host Agent builds the final user-facing answer "
            "from these facts ONLY — never invent jobs, salary, links, "
            "scores, or reasons, and keep every apply URL exactly as "
            "returned as a bare URL. "
            "The radar suppresses previously-seen unchanged jobs; use "
            "include_seen=true for ordinary interactive find/show requests. "
            "Use include_seen=false (default) for explicitly incremental or "
            ...
            "or analysis; same-turn calls are allowed for an explicit "
