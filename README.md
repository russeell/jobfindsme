...
- **你不方便在本机跑 Chrome** —— BOSS 需要登录态，另外三个平台通过本地浏览器读取公开搜索页。
...
| 来源 | 当前方式 | 当前作用 |
|---|---|---|
| BOSS直聘 | 本地登录态浏览器 | 主要实时推荐来源 |
| 猎聘 | 浏览器列表 + 详情页补充 | 补充候选 |
| 智联招聘 | 被动拦截搜索接口 JSON | 补充候选 |
| 前程无忧 | 被动拦截搜索接口 JSON | 补充发现 |
...
**Q：四个平台都要登录吗？**
只有 BOSS 需要（扫码一次，后续复用本地登录态）。猎聘、智联、前程无忧读公开搜索页。
