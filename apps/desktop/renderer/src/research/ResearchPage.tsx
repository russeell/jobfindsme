...
import {acceptsResearchDelta,beginChat,failChat,finishChat,fromStoredResearchChat,loadResearchChats,mergeResearchChats,saveResearchChats,toStoredResearchChat,type ActiveResearchRequest,type SavedResearchChat} from "../../../shared/research-chat-history";
...
export function ResearchPage({active,data,target,onBack,onError,onReports}:Props){
  ...
  useEffect(()=>{if(!workspaceId)return;let cancelled=false;const cached=loadResearchChats(workspaceId);setLoadedWorkspace(null);setChats(cached);setReports([]);setChatId(null);setChatBusy(false);setStreaming("");streamingRef.current="";setModelId(getCurrentModel(workspaceId));void window.jobfindsme!.listResearchChats(workspaceId).then(rows=>{if(cancelled)return;setChats(mergeResearchChats(cached,rows.map(fromStoredResearchChat)));setLoadedWorkspace(workspaceId);}).catch(()=>{if(!cancelled){setLoadedWorkspace(workspaceId);setMessage("对话记录暂未从本地服务加载；当前显示设备缓存。");}});return()=>{cancelled=true;const active=requestRef.current;if(active?.workspaceId===workspaceId){requestRef.current=null;void(active.kind==="model"?window.jobfindsme!.cancelResearchChat(active.id):window.jobfindsme!.cancelResearch());}};},[workspaceId]);
  ...
  return <div className="research-page research-workbench">
    {active&&topbarTarget&&createPortal(<div className="button-row" aria-label="研究操作">
      ...
      <button disabled={chatBusy} onClick={()=>{setChatId(null);setJob(undefined);setContextCompany("");setContextTitle("");setQuestion("");setCandidate(undefined);setReport(undefined);setMode("start");setStreaming("");setMessage("");}}>新对话</button>
