...
export function ResumePage({onChanged}:{onChanged?:(state:ResumeState)=>void}={}){
  ...
  return <div className="resume-page resume-maintenance simple-profile">
    ...
    {draft&&<section className="panel section resume-confirm"><h2>核对新导入的简历</h2><p className="note">{draft.file_name} · {draft.facts.length} 条候选事实。确认前不会用于检索。</p><div className="fact-list">{factGroups.map(type=>{const facts=draft.facts.filter(fact=>fact.fact_type===type);const label=factLabels[type]??"简历信息";return <section className="fact-group" key={type} aria-label={label}><h3>{label}<small>{facts.length} 条</small></h3>{facts.map((fact,index)=><div className="fact-row" key={fact.fact_id}><input type="checkbox" aria-label={`${label} ${index+1}`} checked={accepted.has(fact.fact_id)} onChange={event=>setAccepted(previous=>{const next=new Set(previous);event.target.checked?next.add(fact.fact_id):next.delete(fact.fact_id);return next;})}/><textarea aria-label={`${label} ${index+1} 内容`} value={corrections[fact.fact_id]??fact.value} disabled={!accepted.has(fact.fact_id)||busy} onChange={event=>setCorrections(previous=>({...previous,[fact.fact_id]:event.target.value}))}/></div>)}</section>;})}</div><div className="button-row"><button className="primary-button" disabled={busy||!accepted.size} onClick={()=>void confirm()}>确认简历事实</button><button disabled={busy} onClick={()=>void abandon()}>放弃本次导入</button></div></section>}
