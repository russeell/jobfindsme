...
export function ResearchPage({active,data,target,onBack,onError,onReports}:Props) {
  ...
  function openSaved(value:ResearchReport){lastOpened.current=value.report_id;readSequence.current++;setJob(reportJob(value));setReport(value);setCandidate(undefined);setInputOpen(false);setContextCompany("");setContextDescription("");setInterest(value.job_context?.interest_question||"");setMessage("");setBusy(false);}
  ...
  const [directions,setDirections] = useState<ResearchDirection[]>(Object.keys(researchDirections) as ResearchDirection[]);
  const [interest,setInterest] = useState("");
  const [sourceIds,setSourceIds] = useState(["maimai","offershow","kanzhun","zhihu"]);
  const [busy,setBusy] = useState(false);
  const openBrowser = useOriginalBrowser();
  useEffect(() => { lastOpened.current=undefined; setContextCompany("");setContextDescription("");setInterest("");setJob(target); setCandidate(undefined); setReport(undefined); setInputOpen(!target); setMessage(""); setBusy(false); readSequence.current++; }, [target]);
  ...
  async function run() {
    ...
    const sequence=++readSequence.current;setBusy(true);onError(undefined);
    try {
      const value=await window.jobfindsme!.createResearchReport({workspace_id:workspaceId,job_id:job.job_id,source_ids:sourceIds,user_evidence:[],directions,interest_question:interest.trim()||undefined,context_company:contextCompany||undefined,context_description:contextDescription||undefined});
  ...
  <>{report&&<section className="report-saved" role="status"><strong>研究报告 v{report.version_number??1} · {reportStatus(report)}</strong><p>{new Date(report.created_at).toLocaleString()} · 已保存在本机</p><p className="note">当前展示保存时的岗位信息和证据。再次研究会生成新报告，不覆盖历史。</p><details><summary>报告岗位快照</summary><p>{report.job_context?.company||"公司未知"} · {report.job_context?.title||"历史岗位"}</p><p className="job-description">{report.job_context?.description||"保存时未读取到完整职位描述"}</p><p>{report.job_context?.url||"原链接未知"}</p></details></section>}{report&&<ReputationEvidence report={report} workspaceId={workspaceId!} onReport={value=>{setReport(value);keepReports(reports.map(item=>item.report_id===value.report_id?value:item));}}/>}<details className="panel section research-settings" open={!report}><summary>{report?"调整问题并再次研究":"设置口碑调查问题"}</summary><section className="research-subject"><h2>调查对象</h2><p><strong>{report?.job_context?.company||contextCompany||job.company}</strong> · {report?.job_context?.title||job.title}</p><p className="muted">岗位名称、公司、JD 与原链接已带入，仅用于定位调查对象，不作为员工口碑证据。</p>{(!job.description||job.description.length<80)&&<p className="notice">当前 JD 不完整，可读取原页补全，或展开补充信息。</p>}{["公司未知","未知","BOSS直聘"].includes(job.company)&&!contextCompany&&<p className="notice">公司名称未知，请先读取原页或补充名称后调查。</p>}<div className="button-row"><button disabled={busy} onClick={()=>void readLink(job.apply_url)}>读取原页补全</button><button onClick={original}>查看原始链接 ↗</button></div><details><summary>调查上下文 / 补充信息</summary><p>原链接：{report?.job_context?.url||job.apply_url}</p><p>明确标注的团队：{report?.job_context?.team||"未知；不会根据岗位名称猜测"}</p><label>补充公司名称（可选）<input aria-label="补充公司名称" value={contextCompany} maxLength={300} onChange={e=>setContextCompany(e.target.value)}/></label><label>补充 JD（可选）<textarea aria-label="补充 JD" value={contextDescription} maxLength={30000} onChange={e=>setContextDescription(e.target.value)}/></label><p className="job-description">{report?.job_context?.description||contextDescription||job.description||"JD 未知"}</p></details></section><label className="research-interest">你最想了解什么？<textarea aria-label="自定义调查问题" value={interest} maxLength={300} disabled={busy} onChange={e=>setInterest(e.target.value)} placeholder="例如：这家公司 AI 工程师团队的工作节奏如何？"/><span className="note">填写后优先用这个问题检索；快捷方向仅在未填写时组成检索词。</span></label><fieldset className="research-directions"><legend>快捷方向（可选）</legend>{(Object.entries(researchDirections) as Array<[ResearchDirection,string]>).map(([key,label])=><label key={key}><input type="checkbox" checked={directions.includes(key)} disabled={busy} onChange={e=>setDirections(current=>e.target.checked?[...current,key]:current.filter(item=>item!==key))}/>{label}</label>)}</fieldset><fieldset className="research-directions"><legend>公开来源</legend>{([["maimai","脉脉"],["offershow","OfferShow"],["kanzhun","看准"],["zhihu","知乎"]] as const).map(([id,label])=><label key={id}><input type="checkbox" checked={sourceIds.includes(id)} disabled={busy} onChange={e=>setSourceIds(old=>e.target.checked?[...old,id]:old.filter(item=>item!==id))}/>{label}</label>)}</fieldset><div className="research-actions"><p className="muted">每来源只做一次有限检索并核对少量原页；受限时保留未查明项，不调用模型评价。</p><button className="primary-button" disabled={busy||!sourceIds.length||(!interest.trim()&&!directions.length)||(["公司未知","未知","BOSS直聘"].includes(job.company)&&!contextCompany.trim())} onClick={()=>void run()}>{busy?"检索中…":report?"再次研究 · 保存新报告":"确认并研究"}</button></div></details>{!report&&<ReputationEvidence report={report} workspaceId={workspaceId!} onReport={value=>{setReport(value);keepReports(reports.map(item=>item.report_id===value.report_id?value:item));}}/>}</>
