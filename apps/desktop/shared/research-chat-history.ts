...
export type SavedResearchChat={id:string;title:string;updatedAt:string;turns:ResearchChatTurn[];reportIds:string[];subjectCompany?:string;subjectTitle?:string;draft?:string;failure?:string;pendingResearch?:PendingResearch};
const key=(workspaceId:string)=>`jobfindsme:research-chat:${workspaceId}`;
export type ActiveResearchRequest={id:string;workspaceId:string;kind:"model"|"report"};
export const acceptsResearchDelta=(active:ActiveResearchRequest|null,event:ResearchChatDelta,workspaceId?:string)=>!!active&&active.kind==="model"&&active.id===event.request_id&&active.workspaceId===event.workspace_id&&workspaceId===event.workspace_id;
...
export function beginChat(chat:SavedResearchChat|undefined,id:string,question:string,at:string):{chat:SavedResearchChat;history:ResearchChatTurn[]}{
  ...
  return {history,chat:{id,title:chat?.title||question.slice(0,40),updatedAt:at,turns:retry?turns:[...turns,{role:"user",text:question}],reportIds:chat?.reportIds||[],subjectCompany:chat?.subjectCompany,subjectTitle:chat?.subjectTitle,draft:question,pendingResearch:chat?.pendingResearch}};
}
...
export function saveResearchChats(workspaceId:string,chats:SavedResearchChat[]):boolean{
  try{localStorage.setItem(key(workspaceId),JSON.stringify(chats.slice(0,50).map(chat=>({...chat,turns:chat.turns.slice(-20)}))));return true;}
  catch{return false;}
}

export function toStoredResearchChat(workspaceId:string,chat:SavedResearchChat):Record<string,unknown>{
  return {workspace_id:workspaceId,id:chat.id,subject_key:(chat.subjectCompany||"").toLocaleLowerCase().replace(/\s+/g,""),subject_company:chat.subjectCompany,subject_title:chat.subjectTitle,turns:chat.turns.slice(-40),report_ids:chat.reportIds.slice(-30),draft:chat.draft,pending:chat.pendingResearch,failure:chat.failure};
}
export function fromStoredResearchChat(item:Record<string,unknown>):SavedResearchChat{
  ...
  return {id:String(item.id),title:first?.text.slice(0,40)||"研究对话",updatedAt:String(item.updated_at||new Date().toISOString()),turns,reportIds:Array.isArray(item.report_ids)?item.report_ids.filter((value):value is string=>typeof value==="string"):[],subjectCompany:typeof item.subject_company==="string"?item.subject_company:undefined,subjectTitle:typeof item.subject_title==="string"?item.subject_title:undefined,draft:typeof item.draft==="string"?item.draft:undefined,pendingResearch:item.pending&&typeof item.pending==="object"?item.pending as PendingResearch:undefined,failure:typeof item.failure==="string"?item.failure:undefined};
}
