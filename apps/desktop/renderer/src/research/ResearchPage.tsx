...
import {decideResearchRequest} from "../../../shared/research-dialogue";
...
import {acceptsResearchDelta,beginChat,failChat,finishChat,fromStoredResearchChat,loadResearchChats,saveResearchChats,toStoredResearchChat,type ActiveResearchRequest,type SavedResearchChat} from "../../../shared/research-chat-history";
...
export function ResearchPage({active,data,target,onBack,onError,onReports}:Props){
  ...
  useEffect(()=>{if(!workspaceId)return;let cancelled=false;setLoadedWorkspace(null);setChats(loadResearchChats(workspaceId));setReports([]);setChatId(null);setChatBusy(false);setStreaming("");streamingRef.current="";setModelId(getCurrentModel(workspaceId));void window.jobfindsme!.listResearchChats(workspaceId).then(rows=>{if(cancelled)return;const remote=rows.map(fromStoredResearchChat);if(remote.length)setChats(remote);setLoadedWorkspace(workspaceId);}).catch(()=>{if(!cancelled){setLoadedWorkspace(workspaceId);setMessage("对话记录暂未从本地服务加载；当前显示设备缓存。");}});return()=>{cancelled=true;const active=requestRef.current;if(active?.workspaceId===workspaceId){requestRef.current=null;void(active.kind==="model"?window.jobfindsme!.cancelResearchChat(active.id):window.jobfindsme!.cancelResearch());}};},[workspaceId]);
  useEffect(()=>{if(!workspaceId||loadedWorkspace!==workspaceId)return;if(!saveResearchChats(workspaceId,chats))setMessage("历史对话未能写入设备缓存，请检查可用空间。");for(const chat of chats){saveQueue.current=saveQueue.current.catch(()=>undefined).then(()=>window.jobfindsme!.saveResearchChat(toStoredResearchChat(workspaceId,chat))).catch(()=>{setMessage("对话记录未能写入本地服务，请重试。");});}},[workspaceId,loadedWorkspace,chats]);
  ...
  async function sendChat(value:string){
    ...
    const decision=decideResearchRequest(value,{company:jobCompany||contextCompany||prior?.subjectCompany,title:job?.title||contextTitle||prior?.subjectTitle,hasJob:!!job,pending:prior?.pendingResearch});
    const currentCompany=jobCompany||contextCompany||prior?.subjectCompany;
    ...
    const requestId=crypto.randomUUID();requestRef.current={id:requestId,workspaceId,kind:"model"};
    const startedChat={...started.chat,subjectCompany:decision.kind==="research"?decision.company:current?.subjectCompany,subjectTitle:decision.kind==="research"?decision.title:current?.subjectTitle};
    ...
    try{
      const sameJob=decision.kind==="research"&&!!job&&(unknownCompanies.has(job.company)||decision.company.toLocaleLowerCase()===job.company.toLocaleLowerCase());
      const result=await window.jobfindsme!.runResearchChat({request_id:requestId,workspace_id:workspaceId,connection_id:selected.connection_id,question:decision.kind==="research"?decision.question:value,research:decision.kind==="research",job_id:sameJob?job?.job_id:undefined,company:decision.kind==="research"?decision.company:undefined,title:decision.kind==="research"?decision.title:undefined,history:started.history});
  ...
  async function cancelChat(){const active=requestRef.current;if(!active)return;try{if(active.kind==="model")await window.jobfindsme!.cancelResearchChat(active.id);else await window.jobfindsme!.cancelResearch();setMessage("正在停止；当前提问会保留。");}catch(error){setMessage(userError(error).message);}}
  ...
  const centeredEmpty=!showingReport&&!chatId&&!job&&!contextCompany&&!historyOpen&&!message;
  ...
  return <div className="research-page research-workbench">
    ...
    <div className={`research-scroll-region${centeredEmpty?" research-empty-state":""}`} role="region" aria-label="研究报告正文" tabIndex={0}><div className="research-reading-column">
      ...
      {historyOpen&&<section className="research-history-panel" aria-label="历史对话与报告"><div className="research-history-title"><strong>历史对话</strong></div><div className="research-history-list">{chats.map(item=><div className="research-history-row" key={item.id}><button disabled={chatBusy} aria-pressed={chatId===item.id} onClick={()=>{setChatId(item.id);const latest=[...item.reportIds].reverse().map(id=>reports.find(value=>value.report_id===id)).find(Boolean);setReport(latest);setMode(latest?"report":"start");setJob(latest?.job_id?reportJob(latest):undefined);setContextCompany(item.subjectCompany||"");setContextTitle(item.subjectTitle||"");setQuestion(item.draft||"");setHistoryOpen(false);setMessage(item.failure||"");}}><strong>{item.title}</strong><small>{new Date(item.updatedAt).toLocaleString()} · {item.turns.length} 条消息</small></button></div>)}</div><div className="research-history-title"><strong>历史报告</strong></div>{history.length?<div className="research-history-list">{history.map(item=><div className="research-history-row" key={item.report_id}><button disabled={!!busy||chatBusy} aria-pressed={report?.report_id===item.report_id&&showingReport} onClick={()=>{openSaved(item);setHistoryOpen(false);}}><strong>{item.job_id?`${item.job_context?.company||"公司未知"} · ${item.job_context?.title||"岗位未知"}`:`${item.job_context?.company||"公司未知"} · 公司研究`}</strong><small>{new Date(item.created_at).toLocaleString()} · {reportStatus(item)}{item.job_context?.interest_question?` · 问：${item.job_context.interest_question}`:""}</small></button><button disabled={!!busy||chatBusy} onClick={()=>setHideId(item.report_id)}>移除</button>{hideId===item.report_id&&<div className="history-confirm" role="alert"><p>从列表移除这份报告？本机证据快照保留。</p><div className="button-row"><button disabled={!!busy} onClick={()=>void hideReport(item)}>确认移除</button><button onClick={()=>setHideId("")}>取消</button></div></div>}</div>)}</div>:<p className="research-empty-note">这里还没有报告。提出公司问题即可开始。</p>}</section>}
      {activeChat&&<section className="research-chat-messages" aria-label="对话内容">{activeChat.turns.map((item,index)=><article className={`research-chat-turn ${item.role}`} key={`${activeChat.id}-${index}`}><strong>{item.role==="user"?"你":"研究助手"}</strong><p>{item.text}</p></article>)}{chatBusy&&<article className="research-chat-turn assistant" aria-live="polite"><strong>研究助手</strong><p>{streaming||"正在处理…"}</p></article>}{activeChat.failure&&!chatBusy&&<p className="research-chat-failure" role="status">{activeChat.failure} · 输入框已保留提问，可重试。</p>}</section>}
