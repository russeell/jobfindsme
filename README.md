...
在「岗位研究」输入公司问题即可开始，也可先从岗位带入原始 JD，或粘贴岗位链接。歧义会在对话中追问。选择并测试模型后，研究 Agent 按问题方向复用已有证据、补查缺口、读取原文；重复查询和地址不会反复请求，连续没有新线索会停止发现。公开披露与个人陈述分开呈现，不把零散评价当作整家公司的事实。公开搜索服务故障时只能读取已核验的已知原页；目前没有独立的备用搜索提供方。
...
- 岗位记录、简历、对话和报告保存在本机；访问招聘网站和公开资料需要联网。研究及普通聊天使用你在设置中选择的模型，可能产生模型服务费用；本轮隔离验收没有调用付费模型。自由提问可能成为公开网页检索词，请勿填写隐私信息。
