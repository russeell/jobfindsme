...
import {loadResearchChats,saveResearchChats,type SavedResearchChat} from "./chat-history";
...
export function ResearchPage({active,data,target,onBack,onError,onReports}:Props){
  ...
  function openSaved(value:ResearchReport){sequence.current++;lastOpened.current=value.report_id;setChatId(null);setJob(value.job_id?reportJob(value):undefined);setReport(value);setMode("report");setCandidate(undefined);setContextCompany(value.job_id?"":value.job_context?.company||"");setContextTitle(value.job_id?"":value.job_context?.title||"");setClarify(null);setQuestion("");setMessage("");setBusy(null);}
  useEffect(()=>setTopbarTarget(document.getElementById("research-topbar-actions")),[]);
  useEffect(()=>{if(!workspaceId)return;setChats(loadResearchChats(workspaceId));setChatId(null);setModelId(getCurrentModel(workspaceId));setLoadedWorkspace(workspaceId);},[workspaceId]);
  useEffect(()=>{if(workspaceId&&loadedWorkspace===workspaceId)saveResearchChats(workspaceId,chats);},[workspaceId,loadedWorkspace,chats]);
  ...
  useEffect(()=>window.jobfindsme!.onResearchChatDelta(delta=>{streamingRef.current+=delta;setStreaming(streamingRef.current);}),[]);
  useEffect(()=>{sequence.current++;lastOpened.current=undefined;setChatId(null);setJob(target);setResearchMode(!!target);setReport(undefined);setMode("start");setCandidate(undefined);setContextCompany("");setContextTitle("");setClarify(null);setHistoryOpen(false);setQuestion("");setMessage("");setBusy(null);},[target?.job_id]);
  ...
  async function sendChat(value:string){
    if(!workspaceId||!modelId||chatBusy)return;
    const selected=connections.find(item=>item.connection_id===modelId&&item.status==="verified");
    if(!selected){setMessage("请先在模型设置中保存并测试一个模型。");return;}
    const current=chats.find(item=>item.id===chatId);
    const id=current?.id||crypto.randomUUID();setChatId(id);
    const history=current?.turns||[];
    const userTurn={role:"user" as const,text:value};
    setChats(items=>[{id,title:current?.title||value.slice(0,40),updatedAt:new Date().toISOString(),turns:[...history,userTurn],reportIds:current?.reportIds||[]},...items.filter(item=>item.id!==id)]);
    setQuestion("");streamingRef.current="";setStreaming("");setMessage("");setChatBusy(true);
    try{
      const result=await window.jobfindsme!.runResearchChat({workspace_id:workspaceId,connection_id:modelId,question:value,research:researchMode||!!job,job_id:job?.job_id,company:job?.company&&!unknownCompanies.has(job.company)?job.company:contextCompany||undefined,title:job?.title||contextTitle||undefined,history});
      setChats(items=>items.map(item=>item.id===id?{...item,updatedAt:new Date().toISOString(),turns:[...item.turns,{role:"assistant",text:result.text}],reportIds:result.report?[...item.reportIds,result.report.report_id]:item.reportIds}:item));
      if(result.report){const values=await window.jobfindsme!.listResearchReports(workspaceId);keepReports(values);lastOpened.current=result.report.report_id;setReport(result.report);setMode("report");}
      streamingRef.current="";setStreaming("");
    }catch(error){if(streamingRef.current.trim())setChats(items=>items.map(item=>item.id===id?{...item,turns:[...item.turns,{role:"assistant",text:`${streamingRef.current}\n\n（本次生成中断）` }]}:item));setMessage(`本次对话未完成：${userError(error).message}。已保留提问，可重试。`);try{keepReports(await window.jobfindsme!.listResearchReports(workspaceId));}catch{}}
    finally{setChatBusy(false);}
  }
  async function cancelChat(){if(!chatBusy)return;try{await window.jobfindsme!.cancelResearchChat();setMessage("已请求取消；当前提问已保留。");}catch(error){setMessage(userError(error).message);}}
  ...
  function submitInput(){
    ...
    if(modelId){if(!parsed.question){setMessage("请输入消息，或粘贴岗位链接。");return;}void sendChat(parsed.question);return;}
  ...
  const centeredEmpty=!showingReport&&!chatId&&!job&&!contextCompany&&!historyOpen&&!message&&!clarify;
  ...
  return <div className="research-page research-workbench">
    {active&&topbarTarget&&createPortal(<div className="button-row" aria-label="研究操作">
      ...
      <button disabled={chatBusy} onClick={()=>{setChatId(null);setReport(undefined);setMode("start");setStreaming("");setMessage("");}}>新对话</button>
      <button aria-expanded={historyOpen} onClick={()=>setHistoryOpen(value=>!value)}>历史{history.length+chats.length?` · ${history.length+chats.length}`:""}</button>
    ...
    <form className="research-composer" aria-label="研究输入框" onSubmit={event=>{event.preventDefault();submitInput();}}>
      ...
      <textarea ref={inputRef} rows={2} aria-label="岗位链接与研究问题" value={question} maxLength={700} disabled={!!busy||chatBusy} onChange={event=>{setQuestion(event.target.value);setCandidate(undefined);setMessage("");}} onKeyDown={event=>{if(event.key==="Enter"&&!event.shiftKey&&!event.nativeEvent.isComposing){event.preventDefault();submitInput();}}} placeholder={job?"继续问这个岗位或公司，也可贴链接":"自由对话、研究公司，或粘贴岗位链接"}/>
      ...
      <div className="research-composer-actions"><div className="research-composer-options"><label><input type="checkbox" checked={researchMode||!!job} disabled={!!job} onChange={event=>setResearchMode(event.target.checked)}/> 公开研究</label><select aria-label="当前使用模型" value={modelId||""} onChange={event=>{const value=event.target.value;setModelId(value||null);if(value)setCurrentModel(workspaceId,value);}}><option value="">未选择模型</option>{connections.filter(item=>item.status==="verified").map(item=><option key={item.connection_id} value={item.connection_id}>{item.provider} · {item.model_id}</option>)}</select></div><div className="button-row">{busy==="research"&&<button type="button" onClick={()=>void cancel()}>取消</button>}{chatBusy&&<button type="button" onClick={()=>void cancelChat()}>取消</button>}<button type="submit" className="primary-button" disabled={!!busy||chatBusy||!!candidate}>{busy==="link"?"读取中…":busy==="research"||chatBusy?"生成中…":"发送 ↑"}</button></div></div>
    </form>
  </div>;
}
