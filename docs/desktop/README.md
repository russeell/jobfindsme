...
D53 已完成：研究页顶部操作、正文独立滚动、底部统一输入可随窗口自由伸缩，不固定 1:1；岗位链接与问题合用输入框。设置在侧栏底部，版本见「关于」。当前独立交付为 [D53 应用包](/private/tmp/JobFindsMe-D53-Research.app)，见 [D53 证据](evidence/D53.md)。定时检索继续停用，旧计划和执行记录保留。任务状态以 [tasks.json](tasks.json) 为准。
