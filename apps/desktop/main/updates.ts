...
export async function checkForUpdates(currentTag:string,platform:string,request:typeof fetch=fetch):Promise<UpdateResult>{
  ...
  const compatible=(release.assets??[]).some(asset=>platform==="darwin"?/\.(dmg|zip)$/i.test(asset.name)&&!/win|linux/i.test(asset.name):platform==="win32"?/\.(exe|msi)$/i.test(asset.name):/\.(AppImage|deb|rpm)$/i.test(asset.name));
