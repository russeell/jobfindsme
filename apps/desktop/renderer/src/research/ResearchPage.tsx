...
export function ResearchPage({active,data,target,onBack,onError,onReports}:Props) {
  ...
  return <div className={`research-page ${job ? "has-research-job" : "empty-research"}`}><div className="heading-row"><div><h1>口碑调查</h1><p>{report?"已保存的调查报告":job?`${job.company} · ${job.title}`:"从岗位列表选择对象，或粘贴岗位链接。"}</p></div><div className="button-row"><button onClick={onBack}>← 返回找工作</button>{job && <button onClick={original}>岗位原页 ↗</button>}</div></div>
  {!!reports.length&&<section className="research-history"><button type="button" className="research-history-toggle" aria-expanded={historyOpen} onClick={()=>setHistoryOpen(value=>!value)}>{historyOpen?"▾":"▸"} 历史研究报告 · {history.length}</button>{historyOpen&&<>{job&&<label><input type="checkbox" checked={allHistory} onChange={e=>setAllHistory(e.target.checked)}/> 查看所有岗位</label>}<div className="research-history-list">{history.map(r=><div className="research-history-row" key={r.report_id}><button disabled={busy} aria-pressed={report?.report_id===r.report_id} onClick={()=>{openSaved(r);setHistoryOpen(false);}}>{r.job_context?.company||"公司未知"} · {r.job_context?.title||"历史岗位"}<br/>v{r.version_number??1} · {new Date(r.created_at).toLocaleString()} · {reportStatus(r)}</button><button disabled={busy} onClick={()=>setHideId(r.report_id)}>移除记录</button>{hideId===r.report_id&&<div className="history-confirm" role="alert"><p>从历史列表移除「{r.job_context?.company||"公司未知"} · {r.job_context?.title||"历史岗位"}」v{r.version_number??1}？本机证据与更正保留，不会物理清除。</p><div className="button-row"><button disabled={busy} onClick={()=>void hideReport(r)}>确认移除</button><button disabled={busy} onClick={()=>setHideId("")}>取消</button></div></div>}</div>)}</div></>}</section>}
  <div className="research-context"><span>{report?"":job?`调查对象 · ${job.company} · ${job.title}`:"调查对象"}</span><button onClick={()=>setInputOpen(!inputOpen)}>{inputOpen ? "收起链接输入" : "换一个岗位"}</button></div>
  ...
  {!job ? null : <>
  <>{report&&<section className="report-saved" role="status"><div className="report-summary"><strong>{report.job_context?.company||job.company} · {report.job_context?.title||job.title}</strong><span>v{report.version_number??1} · {new Date(report.created_at).toLocaleString()} · {reportStatus(report)}</span></div><p className="note">已保存在本机 · {report.job_context?.research_topics?.map(topic=>topic==="company"?"公司评价":"岗位情况").join("、")||"历史调查"}</p><details><summary>保存时的岗位信息</summary><p className="job-description">{report.job_context?.description||"保存时未读取到完整职位描述"}</p><p>{report.job_context?.url||"原链接未知"}</p></details></section>}{report&&<ReputationEvidence report={report} workspaceId={workspaceId!} onReport={value=>{setReport(value);keepReports(reports.map(item=>item.report_id===value.report_id?value:item));}}/>}<details className="research-new-run" key={report?.report_id||"new"} open={!report||undefined}><summary>{report?"再次调查 · 保存新报告":"选择调查主题"}</summary><section className="research-compact"><p className="note">岗位与公司自动带入。只选择想了解的主题，可以同时选两项。</p>{["公司未知","未知","BOSS直聘"].includes(job.company)&&<label>补充公司名称<input aria-label="补充公司名称" value={contextCompany} maxLength={300} onChange={e=>setContextCompany(e.target.value)} placeholder="实际公司全称"/></label>}<fieldset className="research-topic-choice"><legend>选择调研主题</legend>{([["company","公司评价","查看公开的正向与负向反馈"],["job","岗位情况","查看工作内容、强度、假期与福利"]] as const).map(([key,label,description])=><label className="research-topic-card" key={key}><input type="checkbox" checked={topics.includes(key)} disabled={busy} onChange={e=>setTopics(old=>e.target.checked?[...old,key]:old.filter(item=>item!==key))}/><span><strong>{label}</strong><small>{description}</small></span></label>)}</fieldset><div className="button-row"><button className="primary-button" disabled={busy||!topics.length||(["公司未知","未知","BOSS直聘"].includes(job.company)&&!contextCompany.trim())} onClick={()=>void run()}>{busy?"调查中…":report?"再次调查 · 保存新报告":"开始调查"}</button></div>{(!job.description||job.description.length<80)&&<p className="note">完整 JD 尚未读取；岗位主题会标明未知。<button disabled={busy} onClick={()=>void readLink(job.apply_url)}>尝试读取岗位原页</button></p>}</section></details></>
