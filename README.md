...
[它解决什么问题](#它解决什么问题) · [快速开始](#快速开始) · [提示词模版](#提示词模版) · [FAQ](#faq) · [English](README.en.md)
...
1. **覆盖更全**：从多个来源找到更多有效、符合条件且不重复的岗位，不用抓取总数冒充覆盖率。
2. **节省时间**：减少平台切换、重复搜索和无关 JD 阅读，尽快给出第一批合格岗位。
3. **推荐更准**：先执行地点、薪资、经验和招聘类型等硬条件，再依据简历证据排序并说明差距。
4. **操作简单**：首次只需“简历路径 + 一句话需求”；以后只需说“继续帮我找新岗位”。

产品是否有用，不看 Connector 数量、工具数量或抓取记录数，而看用户每周发现了多少
**此前没看过、确实匹配、链接有效并值得打开的岗位**。
...
以后：  “继续帮我找新岗位”
        → 复用原有画像和条件 → 只报告新增、变化和重新开放岗位
```

普通用户不需要理解 Workspace、Search Plan、Connector、CDP 或 MCP 参数。每条推荐应固定包含：

```text
岗位｜公司｜城市｜薪资｜社招/校招｜正式/实习
匹配分与证据置信度
为什么适合你
主要差距或信息缺失
新增/变化/已看状态
...
匹配分是可解释的排序分，不是录用概率；信息不足时必须明确标注，不能猜测。
...
- BOSS直聘可以获取较丰富的列表字段；猎聘和智联只对少量候选做有界详情补全；
  前程无忧和拉勾目前仍主要承担岗位发现，详情和招聘类型需要继续增强。
...
| **猎聘** | 岗位发现 + 有界详情补全（候选最多 3 条） | 浏览器桥 |
| **前程无忧** | 岗位发现；详情弹窗提取尚未完成 | 浏览器桥 |
| **智联招聘** | 岗位发现 + 有界详情补全（候选最多 3 条） | 浏览器桥 |
