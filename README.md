...
每次结果包含有界结构化事实（`structuredContent.jobs`）和五段事实基线
（简历解析 / 检索概览 / 过滤说明 / 岗位列表 / 说明），Agent 可以调整措辞和排版，但不得修改事实：
...
推荐岗位至少应保留：**事实行、投递链接、推荐理由**。使用简历时再提供匹配度和证据；不使用简历时仅按明确条件过滤，不虚构匹配度。
匹配度 = 60% 硬条件底分 + 最高 40% 信号加成（技能 > 经验 > 学历 > 活跃度 > 薪资可见），
所有能展示的岗位都已在 60%–100% 区间内，且按匹配度从高到低排列。
事实、链接、基础推荐理由和风险由 Server 提供；匹配度由 Server 在有简历时确定生成。
...
Agent (Claude/GPT/Qwen/WorkBuddy — 负责交互与后续解释)
  ...
  → 增量雷达（新增/变化/重开/关闭识别）
  → Server 返回有界事实 + 五段事实基线；Agent 基于事实组织回答
    （不得编造事实或丢失投递链接）
```
