...
export function App() {
  ...
  const [returnPage,setReturnPage]=useState<WorkPage>("discover");
  function openSettings(tab:SettingsTab){if(page!=="settings")setReturnPage(page);setSettingsTab(tab);setPage("settings");}
  ...
  return <Workbench onError={setError} sidebar={<>
    ...
    <section className="main"><header className="topbar"><span>工作空间 / {page==="settings"?"设置":navItems.find(([,target])=>target===page)?.[0]}</span><div id="research-topbar-actions" className="research-topbar-actions"/><span className="pill">本地数据 · {data?.workspaces.length??0} 个工作空间</span><BrowserToggle/></header><div className={page==="research"?"content research-content":"content"}>
      ...
      {page==="settings"&&<section className="settings-page"><header className="settings-page-header"><div><h1>设置</h1><p>管理招聘来源与模型连接。</p></div><button onClick={()=>setPage(returnPage)}>← 返回{navItems.find(([,target])=>target===returnPage)?.[0]}</button></header><nav className="settings-tabs" aria-label="设置分类">{settingsItems.map(([label,tab])=><button key={tab} className={settingsTab===tab?"active":""} aria-current={settingsTab===tab?"page":undefined} onClick={()=>setSettingsTab(tab)}>{label}</button>)}</nav><div className="settings-panel">{settingsTab==="sources"?<SourcesPage selected={chosenSources} onSelect={chooseSource} data={data} onRefresh={setData} onError={setError}/>:settingsTab==="models"?<ModelsPage workspaceId={data?.workspaces[0]?.workspace_id} onError={setError}/>:<section className="section"><h2>关于 JobFindsMe</h2><p>版本：{buildInfo.label}</p><p>本地数据 · {data?.workspaces.length??0} 个工作空间</p></section>}</div></section>}
    </div></section>
  </Workbench>;
}
...
function SourcesPage({ data, selected, onSelect, onRefresh, onError }: { selected:string[]; onSelect(id:string, selected:boolean):void; data?: BootstrapData; onRefresh(data: BootstrapData): void; onError(message?: string): void }) {
  ...
  return <>
    ...
    {audit&&<div className="source-audit-progress" role="status"><span>{audit.running?audit.cancelled?"正在停止":"检查中":audit.cancelled?"部分完成":"检查结束"} · 已处理 {audit.done}/{audit.total} 个来源{!audit.running&&` · 本次实际探测 ${audit.rows.filter(row=>row.evidence==="live").length} 个，通过 ${audit.rows.filter(row=>row.outcome==="verified_now").length} 个`}</span>{audit.running&&<progress value={audit.done} max={audit.total} aria-label="来源检查进度"/>}</div>}
...
function ModelsPage({ workspaceId, onError }: { workspaceId?:string; onError(message?: string): void }) {
  ...
  return <><div className="heading-row"><div><h1>模型设置</h1><p>选择服务、填写密钥和模型，然后保存并测试。密钥仅存放在系统安全存储。</p></div><span className={secure?"pill ready":"pill blocked"}>{secure?"安全存储可用":"安全存储不可用"}</span></div><div className="settingssplit"><form className="panel model-form" onSubmit={event=>void save(event)}>
    ...
    <button className="primary-button" disabled={!!testing||(!secure&&Boolean(form.api_key))}>{testing?"测试中…":"保存并测试"}</button><p className="note">测试会向所选服务发送一次最小请求；服务商可能计费。</p>{notice&&<p role="status" className="note">{notice}</p>}
