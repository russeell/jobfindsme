...
export function App() {
  ...
  return <Workbench onError={setError} sidebar={<>
    ...
    <section className="main"><header className={page==="settings"?"topbar settings-topbar":"topbar"}><span>工作空间 / {page==="settings"?"设置":navItems.find(([,target])=>target===page)?.[0]}</span>{page==="settings"&&<nav className="settings-tabs" aria-label="设置分类">{settingsItems.map(([label,tab])=><button key={tab} type="button" className={settingsTab===tab?"active":""} aria-current={settingsTab===tab?"page":undefined} onClick={()=>setSettingsTab(tab)}>{label}</button>)}</nav>}<div id="research-topbar-actions" className="research-topbar-actions"/><span className="pill">本地数据 · {data?.workspaces.length??0} 个工作空间</span><BrowserToggle/></header><div className={page==="research"?"content research-content":"content"}>
