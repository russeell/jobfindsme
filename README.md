...
- **你不方便登录 BOSS** —— BOSS 是岗位最多的来源，不登录会少一大块结果（其余四个平台不受影响）。
...
### 第 2 步：登录 BOSS（一次性，推荐）

```bash
jobfindsme setup   # Chrome 自动打开登录页，微信扫码 30 秒搞定
```

![BOSS直聘登录](docs/boss-login.png)

> [!IMPORTANT]
> **只有 BOSS直聘需要登录**，登录态保存在本地，以后搜索自动生效。猎聘、前程无忧、智联、拉勾装完就能搜，无需任何 setup。
...
**Q：搜不到 BOSS 岗位？**
九成是没登录。运行 `jobfindsme setup`，微信扫码，30 秒搞定，以后不用再登。也可以跑 `jobfindsme doctor` 诊断登录态。
