"""Bounded 猎聘 CDP fallback and detail enrichment.

Shares the same Chrome CDP session used by BossZhipinConnector.
The maintained primary path is pure HTTP. This module is used only when an
authorized local Chrome session is available and HTTP discovery fails.
"""
