...
推荐理由由 jobfindsme 基于硬条件和结构化岗位信号生成，Agent 可在这些证据上补充语义判断。
...
Agent 只能基于返回证据补充判断，不得删除或改写上述字段。
...
**当前只维护两个已验证来源**：BOSS直聘 + 猎聘。项目优先保证每个来源
能稳定返回有效岗位，不用名义上的平台数量冒充覆盖率。

| 来源 | 方式 | 速度 | 需要浏览器？ |
|---|---|---|---|
| **BOSS直聘** | 用户授权的本地 Chrome CDP，在页面上下文执行站点请求 | ~0.5s | ✅ 需要（登录态） |
