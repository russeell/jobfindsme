...
export function ResumePage() {
  ...
  function showPreview(version:ResumeVersion){setViewId(version.version_id);setEditing(false);setPreviewOpen(true);requestAnimationFrame(()=>exportDetails.current?.scrollIntoView({behavior:"smooth",block:"start"}));}
  ...
  async function hide(version:ResumeVersion){if(!state?.workspace_id||version.is_current)return;setBusy(true);setHideError("");try{
  ...
  }catch(reason){setHideError(errorText(reason));}finally{setBusy(false);}}
  ...
  return <div className="resume-page resume-maintenance">
    ...
    {draft&&<section className="panel section resume-confirm"><h2>核对解析结果</h2><p className="note">{draft.file_name} · {draft.facts.length} 条候选事实。未确认内容不会参与检索。</p><div className="fact-list">{draft.facts.map(fact=><label className="fact-row" key={fact.fact_id}><input type="checkbox" checked={accepted.has(fact.fact_id)} onChange={event=>setAccepted(old=>{const next=new Set(old);event.target.checked?next.add(fact.fact_id):next.delete(fact.fact_id);return next;})}/><span>{factLabels[fact.fact_type]??"简历信息"}</span><textarea value={corrections[fact.fact_id]??fact.value} disabled={!accepted.has(fact.fact_id)||busy} onChange={event=>setCorrections(old=>({...old,[fact.fact_id]:event.target.value}))}/></label>)}</div><div className="button-row"><button className="primary-button" disabled={busy||!accepted.size} onClick={()=>void confirm()}>确认当前简历</button><button disabled={busy} onClick={()=>void run(async()=>{await window.jobfindsme!.abandonResume(draft.workspace_id,draft.profile_id);await reload();})}>放弃本次导入</button></div></section>}
    {current&&<><section className="panel section resume-current"><div className="heading-row"><div><h2>当前简历</h2><p className="note">{new Date(current.created_at).toLocaleString()} · 已确认并用于下次岗位检索</p></div><div className="button-row"><button onClick={()=>showPreview(current)}>预览 / 导出</button><button className="primary-button" disabled={busy} onClick={()=>{setContent(current.content);setEditing(true);}}>维护内容</button></div></div>
      {editing?<div className="resume-editor">{sections.map(([key,label])=><label className="editor-section" key={key}>{label}<textarea value={content[key].join("\n")} disabled={busy} onChange={event=>setContent(old=>({...old,[key]:lines(event.target.value)}))}/></label>)}<div className="button-row"><button className="primary-button" disabled={busy||!dirty} onClick={()=>void save()}>保存为新版本</button><button disabled={busy} onClick={()=>{setContent(current.content);setEditing(false);}}>取消</button></div><p className="note">每行一条内容；保存前不会改变当前版本，原文件仍保留。</p></div>:<div className="resume-summary">{sections.filter(([key])=>key!=="basic_information"||current.content[key].some(line=>line.trim())).map(([key,label])=><section key={key}><h3>{label}</h3>{current.content[key].length?current.content[key].map((line,index)=><p key={index}>{line}</p>):<p className="muted">暂无内容</p>}</section>)}</div>}
    </section><details className="panel section resume-history"><summary>版本历史 · {versions.length}</summary><div className="version-list">{versions.map(version=><div className="resume-history-row" key={version.version_id}><div className="resume-history-main"><span>v{version.version_number} · {new Date(version.created_at).toLocaleString()} {version.is_current&&<em>当前</em>}</span><div className="button-row"><button onClick={()=>showPreview(version)}>预览</button>{!version.is_current&&<><button disabled={busy} onClick={()=>void restore(version)}>恢复为新版本</button><button disabled={busy} onClick={()=>{setHideError("");setHideId(version.version_id);}}>移除记录</button></>}</div></div>{hideId===version.version_id&&<div className="history-confirm" role="alert" onKeyDown={event=>{if(event.key==="Escape"){event.preventDefault();setHideId("");setHideError("");}}}><p>从历史列表移除 v{version.version_number}？已有引用保留。</p>{hideError&&<p className="notice">{hideError}</p>}<div className="button-row"><button autoFocus disabled={busy} onClick={()=>void hide(version)}>确认移除</button><button disabled={busy} onClick={()=>{setHideId("");setHideError("");}}>取消</button></div></div>}</div>)}</div><p className="note">恢复会创建新版本；被已有记录引用的版本无法移除。</p></details><details ref={exportDetails} open={previewOpen} onToggle={event=>setPreviewOpen(event.currentTarget.open)} className="panel section resume-export"><summary>预览与导出已保存版本</summary><label>版本 <select value={viewed?.version_id||""} onChange={event=>setViewId(event.target.value)}>{versions.map(version=><option key={version.version_id} value={version.version_id}>v{version.version_number}{version.is_current?" · 当前":""}</option>)}</select></label><label>排版 <select value={template} onChange={event=>setTemplate(event.target.value as "classic"|"compact")}><option value="classic">经典</option><option value="compact">紧凑</option></select></label><div className={`resume-preview ${template}`}><h2>简历 · v{viewed?.version_number}</h2>{sections.filter(([key])=>key!=="basic_information"||!!viewed?.content[key].some(line=>line.trim())).map(([key,label])=><section key={key}><h4>{label}</h4>{viewed?.content[key].map((line,index)=><p key={index}>{line}</p>)}</section>)}</div><div className="button-row">{(["pdf","docx","md"] as const).map(format=><button key={format} disabled={busy} onClick={()=>void exportFile(format)}>导出 {format.toUpperCase()}</button>)}</div></details></>}
  </div>;
}
